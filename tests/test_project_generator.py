...
def test_generate_project_with_new_structure(tmp_path: Path) -> None:
    ...
    assert (target / "app" / "rag" / "chunker.py").exists()
    assert (target / "app" / "rag" / "hybrid.py").exists()
    assert (target / "app" / "rag" / "reranker.py").exists()
    assert (target / "app" / "plugins" / "loader.py").exists()
    assert (target / "app" / "plugins" / "sandbox.py").exists()
    assert (target / "app" / "plugins" / "runner.py").exists()
    assert (target / "app" / "plugins" / "audit.py").exists()
    assert (target / "app" / "plugins" / "policy.py").exists()
    assert (target / "app" / "policy" / "engine.py").exists()
    assert (target / "app" / "policy" / "policies.json").exists()
    assert (target / "app" / "agents" / "contracts.py").exists()
    assert (target / "app" / "agents" / "orchestrator.py").exists()
    assert (target / "app" / "models" / "router.py").exists()
    assert (target / "app" / "models" / "cost_guard.py").exists()
